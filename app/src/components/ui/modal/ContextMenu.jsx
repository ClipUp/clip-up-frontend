...
import { useContextMenuStore, useProfileContextMenuStore } from '../../../store/modalStore.js';
...
const ProfileContextMenuItem = ({ imageUrl, title, onClick }) => {
  const { closeContextMenu } = useProfileContextMenuStore((state) => state);
  const handleClick = () => {
    if (onClick) onClick();
    closeContextMenu();
  }
...
const ProfileContextMenu = () => {
  const { contextMenu, closeContextMenu } = useProfileContextMenuStore((state) => state);
  const contextMenuRef = useRef(null);
...
const ProfileContextMenuButton = ({ id, imageUrl, closeImageUrl, menuList }) => {
  const { openContextMenu, closeContextMenu, contextMenu } = useProfileContextMenuStore((state) => state);
...
export { ContextMenu, ContextMenuButton, ContextMenuItem, ProfileContextMenu, ProfileContextMenuButton, ProfileContextMenuItem };